...
const AllStudents: React.FC = () => {
  ...
  const [isSubmitAttendanceModalOpen, setIsSubmitAttendanceModalOpen] =
  ...
  const [studentsAttendanceData, setStudentsAttendanceData] =
    useState<StudentAttendanceDataProps[] | null>();
  ...
  const openSubmitAttendanceModal = async () => {
    try {
      setIsSubmitAttendanceModalOpen(true);
      const response = await GetStudentsAttendanceData();
      if (response != null) {
        setStudentsAttendanceData(response);
      }
    } catch (error) {
      console.error("Failed to load notice data", error);
    }
  ...
  const closeSubmitAttendanceModal = () => {
    setIsSubmitAttendanceModalOpen(false);
  };
  ...
  return (
    <>
      ...
      <Box sx={{ marginTop: 1 }}>
        ...
        >
          ...
          <Box sx={{ display: "flex", gap: 2 }}>
            <Button
              onClick={openSubmitAttendanceModal}
              sx={{
        ...
        <Box className="table-responsive" sx={{ marginTop: 3 }}>
          <table
            ...
            className="student-table table table-responsive"
          >
            <thead>
              <tr>
                <th>StudentId</th>
              ...
              <tr>
                <CustomTablePagination
                  rowsPerPageOptions={[
                    ...
                    10,
                    25,
                    { label: "All", value: studentData?.totalItems ?? 0 },
                  ]}
                  // colSpan={3}
      ...
      <SubmitAttendanceModal 
      isOpen={isSubmitAttendanceModalOpen}
      onClose={closeSubmitAttendanceModal}
      initialData={studentsAttendanceData}
      />
    </>
  );
};
